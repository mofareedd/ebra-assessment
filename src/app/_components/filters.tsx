      ...
      <ScrollArea className="">
        <div className="flex py-6 flex-col items-start space-y-3">
          ...
          {categories.map((c) => (
            <Link
              key={c}
              href={""}
              className="text-muted-foreground font-medium border-b border-transparent hover:text-foreground hover:border-b-foreground"
            >
              {c}
            </Link>
          ))}
        ...
        <div className="flex py-6 flex-col items-start space-y-3">
          ...
          <div className="w-full flex flex-col space-y-3">
            ...
            {PRICES.map((p) => (
              <div key={p.val} className="flex items-center justify-between">
